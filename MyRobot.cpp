...
public:
    ...
    stick2(Consts::STICK2_PORT)
  {
    ...
    // feeder object
    myFeeder = new TorFeeder(*ds, *driverStation);
    ...
    //PIDs
    myShooterArmPID = new PIDController(0.025,0.0013,0.0, myShooterArmPOT, myShooterArmJag, 0.05);     // PID controller for moving the shooter
  ...
  void Autonomous(void)
  {
    ...
    case (1): // first tick on driver control dial
             ...
             // move 24 inches forward, raise arm an move another 24 inches to get into firing position
                 myTorbotDrive->DriveStraight(0.5, 10.0);
    ...
    case (3):  // third tick on driver control dial //JUST SHOOT
            ds->Printf(DriverStationLCD::kUser_Line2, 1, "Auto: just shoot...");
            ds->UpdateLCD();
        //    
        //    
            myFeeder->InitalizeAuto(3); // init 2 disks in feeder; really only 3 but we'll shoot max number of times in case of jam
           // myTorbotDrive->DriveStraight(0.25, 14.0);

            myShooter->SetArmPosition(TorShooter::Targeting); //move arm to position
            myShooterArmPID->SetSetpoint(myShooter->GetArmSetPoint()); //set PID
            Wait(3.0); //Wait for camera to adjust
            if (myShooter->Target())  //checks if any target has been found, does not look for specific targets. Closest to center of camera view is targeted
                {
                myShooterArmPID->SetSetpoint(myShooter->GetArmSetPoint());                   // move arm to target
                TurnToTheta(myShooter->GetTurnSetPoint(), true);                                // set robot turn setpoint and wait for it to reach position
                Wait (1.0);                                                                 // give a little extra time for arm to settle in at setpoint
    ...
    case (4): // fourth tick on driver control dial
        ...
        //
           // TorTest test (stick2, ds, myJagDrive);
           // test.testTorClimber();
        //    12 inches forward
    ...
    while (IsOperatorControl() && IsEnabled())
      {
        ...
        ds->Printf(DriverStationLCD::kUser_Line4, 1, "Pot: %d", myShooterArmPOT->GetAverageValue());
       ...
       ShooterArmManualControl();                                                      // stick2 Y-axis controls shooter arm motion manually
        ...
        // check buttons on stick1 for manually resetting the feeder and disk array
        if (stick1.GetRawButton(3))
          {
            myFeeder->resetDisks();                                             // resets the feeder position and zeros out the array
          }

        if (stick1.GetRawButton(5))
          {
            myFeeder->InitalizeAuto(4);                                         // makes the robot think it has 4 disks in case there are extras that need to be shot
          }
        if (stick1.GetRawButton(7))
          {
            hangSolenoid->Set(true);
          }
        if (stick1.GetRawButton(9))
          {
            hangSolenoid->Set(false);
          }
        if (stick1.GetRawButton(Consts::FORKLIFT_LIFT_BUTTON))
          {
            myClimber->liftHands(0.5);
          }
        else if (stick1.GetRawButton(Consts::FORKLIFT_PULL_BUTTON))
          {
            myClimber->pullHands();
          }
        else
          {
            myClimber->liftHands(0.0);
          }
        
        //        for (int slot = 1; slot<=4; slot++)
        //          {
        //                driverStation->SetDigitalOut(slot+4, myFeeder->getFeeder(slot));
        //          }
