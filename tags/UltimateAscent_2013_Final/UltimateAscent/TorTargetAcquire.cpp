  ...
  if (!particles->empty())
    {
      ...
      m_target->yThetaAdjusted = m_target->yTheta*1.2;                  // adjust 20% higher for upside down targets
...
//      m_target->distance = particles->size(); // debug code until we have a reason to return Distance
      ...
      // estimate distance based on size of the image
      if (scores[0].imageWidth > 220)      //  distance is 0-7ft
...
//            scores[i].yEdge = scoreYEdge(initialFilterImage, report);
            ...
            scores[i].yTop = report->center_mass_y-(report->boundingRect.height/2.0)/120.0; // distance to top from center, normalized
            scores[i].imageWidth = report->boundingRect.width;        //                particleToImagePercent;
