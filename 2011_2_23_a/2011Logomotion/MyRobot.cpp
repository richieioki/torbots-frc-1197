...
int minArmAngle = 15; //30; //farthest the arm should go (degrees)
int maxArmAngle = 270; //farthest the arm should go (degrees)
int bottomPegAngle = 33 + minArmAngle; //# of degrees for the pegs, relative from minArmAngle
int middlePegAngle = 65 + minArmAngle;
int topPegAngle = 100 + minArmAngle;
...
int encoderTicks = 250;//360; //tick count on the encoder
...
public:
	RobotDemo(void)
	{
		...
		armDeploymentPiston = new Solenoid(7,1);	// CRIO Solenoid 1
		relayPistonIn = new Solenoid(7,2);			// CRIO Solenoid 2
		...
		while (IsAutonomous()) //Storyship Entorprise
		{
			...
			{	
				...
				case AUTORaiseArm:
					...
					turnPid->Disable();
					pid1->SetTolerance(pidTolerance + 0.5f);
					pid2->SetTolerance(pidTolerance + 0.5f);
					pid1->Enable();
					pid2->Enable();
					...
					AUTOState = AUTOWait; //wait...
					AUTOWaitTime = 0.05; //...for 1 second...
					AUTOWaitNextState = AUTOCheckArmRaise; //...before approaching the peg
					timer->Start(); //start timer
					timer->Reset(); //reset timer to 0
					break;
				...
				case AUTOApproachPeg:
					...
					else
					{
						...
						moveSpeed = 0.0;
						AUTOState = AUTOWait; //wait...
						AUTOWaitTime = 0.2; //...for 0.5 second...
				...
				case AUTORelease:
					...
					pid1->Enable();
					pid2->Enable();
					pid1->SetSetpoint(((0.65*(topPegAngle-middlePegAngle))+middlePegAngle)*1000/maxPotAngle);
					pid2->SetSetpoint(((0.65*(topPegAngle-middlePegAngle))+middlePegAngle)*1000/maxPotAngle);
					AUTOState = AUTOWait; //wait...
