...
float TorbotDrive::getDistance() {
  ...
  encoderDistance = (float)m_encoder.GetRaw()*Consts::wheelCircumference*Consts::wheelGearRatio_High/(Consts::encoderTicks*4.0);
...
//float TorbotDrive::TurnToTheta(float motorSpeed, float destTheta, bool waitFinish)
//{
//
//  double targetAngle = destTheta; 
//
//  targetAngle += m_gyro.GetAngle();      // add the current angle to the target angle to adjust for any gyro error
//
...
////  while (waitFinish && (fabs(m_gyro.GetAngle()) < (fabs(targetAngle)*(1.01))))  // 1% tolerance
////    {
...
// end the turn by disabling the turnPID controller
//void TorbotDrive::disableTurnPID()
//{
//  turnPID->Disable();
//}
