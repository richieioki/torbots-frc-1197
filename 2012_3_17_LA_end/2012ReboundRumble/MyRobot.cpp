...
public:
	...
	static const int driveToFenderState = 1;
		static const float distToFender = 104; // inches
	//State 2
...
public:
	...
	//								   Circ      spkt     count
	static const float encoderToDist = 25.13 / ((26/15) * 440); // converts encoder ticks to distance (inches)
	...
	static const float highKeyShootSpeed = -1.0;
...
public:
	...
	void setDrivePID(float rotateValue, float moveValue, bool usePID) //move and rotate switched
	{
		...
		leftMotorOutput = (-1 * abs(stick.GetY())) - stick.GetX();
		rightMotorOutput = abs(stick.GetY()) - stick.GetX();
	...
	{		
		...
		while(IsAutonomous()){
			...
			else
			{
				leftDrivePID->Enable();
				rightDrivePID->Enable();
				setDrivePID(driveSpeed, -1 * driveTurn, stick.GetZ() < 0);
			}
			
			elevatorJag->Set(-1 * conveyorSpeed);			
